...
#'    - lme: model estimates and significance
...
kmFit_lme <- function(dat=NULL, kin=NULL, patientID="ptID", libraryID="libID",
  ...
  fit.results <- data.table::rbindlist(fill=TRUE,
    ...
    if(any(is.character(results.lme.ls[["results"]]$estimate),
    ...
    fit.results <- results.lme.ls[["results"]] %>%
